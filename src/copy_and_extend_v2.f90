...
SUBROUTINE stop_timer
    ...
    write(lurep, * ) timeused
    write(lurep, * ) timeused_new
    write(lurep, * ) counts
    write(lurep, * ) counts_new
...
PROGRAM copy_and_extend
    ...
    INTEGER, ALLOCATABLE :: array(:), chunk(:)

    INTEGER              :: i, repeat, chunk_size, chk, appends, total, repeats
    INTEGER              :: factor(0:5) = [ 1000, 300, 100, 30, 1, 1]


    OPEN( NEWUNIT = lurep, FILE = 'copy_and_extend_v2.out' )
    OPEN( NEWUNIT = lucsv, FILE = 'copy_and_extend_v2.csv' )
    ...
    DO chk = 0,4 !5
        ...
        DO repeat = 1,repeats
            ...
            DO i = 1,appends
                array = [array, chunk]
            ENDDO
            total = array(1) + array(size(array))
