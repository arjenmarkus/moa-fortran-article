...
PROGRAM moa_measure
    ...
    !
    ! Defaults (mainly for postprocessing)
    !
    number_chunks = -1
    chunk_size    = -1
    array_size    = -1

    CALL system_clock( count = time_zero )
    ...
    DO WHILE ( parser%has_next() )
        ...
        SELECT CASE( command )
            CASE( 'report-file' )
                ...
                OPEN( NEWUNIT = lucsv, FILE = trim(filename) // '.csv' )
    ...
    IF ( array_size > 0 ) THEN
        CALL write_csv_overall( array_size, 1 )
    ELSE
        CALL write_csv_overall( chunk_size, number_chunks )
    ENDIF
    WRITE(*,'(a)') 'Done'
...
SUBROUTINE random_get
    ...
    !do i = 1,size(x%prev_elem)
    !    write(*,*) i, x%prev_elem(i)
    !enddo
